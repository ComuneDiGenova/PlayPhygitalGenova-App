...
public class OnlineMapsLocationService : OnlineMapsLocationServiceGenericBase<OnlineMapsLocationService>
{
    ...
    public void StartLocationService(float? desiredAccuracyInMeters = null, float? updateDistanceInMeters = null)
    {
        ...
        Debug.LogWarning("Start Location");
...
/*
    ...
    public override bool TryStartLocationService()
    {
        ...
        if(!isPermissionRequested){
            ...
            callbacks.PermissionGranted += (content) =>
            {
                Debug.LogWarning("Permission granted " + content);
                StartLocationService();
            };
            callbacks.PermissionDenied += (content) =>
            {
                Debug.LogWarning("Permission DENIED " + content);
            };
            callbacks.PermissionDeniedAndDontAskAgain += (content) =>
            {
                Debug.LogWarning("Permission DENIEND NOT ASK " + content);
            };
            Debug.LogWarning("REQUEST MAP LOCATION PERMISSION");
